...
item_scroller_mt= {
	__index= {
		...
		get_current_index=function(self)
			return self:maybe_wrap_index(self.info_pos, self.focus_pos,self.info_set)
		end;
		get_info_at_offset_from_current=function(self,offset)
			local index = self:get_current_index()
			--[[
			...
			if index+offset > #self.info_set then
				return self.info_set[offset-(#self.info_set-index)]
			--[[
			...
			elseif index+offset < 0 then
				return self.info_set[#self.info_set-offset+index]
			else
				return self.info_set[index+offset]
			end;
		end;

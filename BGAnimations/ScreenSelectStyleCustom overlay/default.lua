local t = Def.ActorFrame {
	...
	LoadActor("base")..{InitCommand=cmd(FullScreen);};
	LoadActor(THEME:GetPathG("ScreenWithMenuElements","header"))..{InitCommand=cmd(addy,20)};
};
  ...
  Def.Sprite{
    ...
    SetCommand=function(self)
      ...
      if GetP1 == true and GAMESTATE:GetMasterPlayerNumber() == PLAYER_1 then
        self:visible(true)
        self:Load(THEME:GetPathB("","ScreenSelectStyleCustom overlay/P1here"));
      elseif GetP1 == false and GAMESTATE:PlayersCanJoin() and GAMESTATE:GetMasterPlayerNumber() == PLAYER_2 then
        self:visible(true)
        self:Load(THEME:GetPathB("","ScreenSelectStyleCustom overlay/P1CanJoin"));
  ...
  Def.Sprite{
    ...
    SetCommand=function(self)
      ...
      if GetP2 == true and GAMESTATE:GetMasterPlayerNumber() == PLAYER_2 then
        self:visible(true)
        self:Load(THEME:GetPathB("","ScreenSelectStyleCustom overlay/P2here"));
      elseif GetP2 == false and GAMESTATE:PlayersCanJoin() and GAMESTATE:GetMasterPlayerNumber() == PLAYER_1 then
        self:Load(THEME:GetPathB("","ScreenSelectStyleCustom overlay/P2CanJoin"));
        self:visible(true)
      elseif GAMESTATE:GetNumPlayersEnabled() == 2 then
        self:visible(false)
      end;
